...
button?.addEventListener('click', (event) => {
    ...
    const imageRandom = Math.floor(Math.random() * data.length);
    // console.log(data[imageRandom]);
    
    image.innerHTML = "";

    const imgElement = document.createElement('img');

    imgElement.src = data[imageRandom].download_url;
    imgElement.alt = data[imageRandom].author;
